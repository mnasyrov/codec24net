			...
			float deltat[])
{
    ...
    for(i=0; i<RES_POINTS; i++) {
	Ares[i] = pow((float)10.0,sample_log_amp_quad_nl(w, A, model->L, wres[i]));
    }
...
float resample_amp_nl(MODEL *model, int m, float AresdB_prev[])
{
    ...
    for(i=1; i<model->L; i++) {
	new_A = pow((float)10.0,sample_log_amp_quad_nl(wres, Ares, RES_POINTS, model->Wo*i));
	signal += pow(model->A[i], (float)2.0);
	noise  += pow(model->A[i] - new_A, (float)2.0);
	//printf("%f %f\n", model->A[i], new_A);
	model->A[i] = new_A;
    }
